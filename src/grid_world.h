...
class grid_world : public i_world  {
public:
    grid_world( int w , int h,std::shared_ptr<i_logger> logger = nullptr );
  
    void init() override ;


    bool is_free(Position p) const override ;
    bool move_entity(std::shared_ptr<i_entity> entity, Position new_pos) override ;
    bool remove_entity(std::shared_ptr<i_entity> e) override ;
    bool add_entity(std::shared_ptr<i_entity> e) override ;
