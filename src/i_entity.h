...
public:
    ...
    virtual bool try_spawn(std::shared_ptr<IWorld> world, Position p)=0;

    /// @brief entity can have an update method to update its state
    virtual void update(std::shared_ptr<IWorld> world)=0;

    /// @brief Terminate the entity
    virtual void despawn(std::shared_ptr<IWorld> world)=0;
    ...
    virtual bool try_move(std::shared_ptr<IWorld> world, Position p)=0;
